...
def predict_disease_csv(csv_file):
    with open('model-pickle-RandomForest-Knn-Svc','rb') as file:
...
def homepage():
    ...
    if select=="Enter details individually":
        ...
        with st.form(key='my_form'):
            ...
            trestbps=col1.number_input("Enter resting blood pressure (in mm Hg on admission to the hospital)",step=1,min_value=60,max_value=250)
            ...
            thalach=col2.number_input("Enter maximum heart rate achieved",step=1,min_value=60, max_value=180)
            ...
            thal=col2.selectbox("Enter the thalassemia value",("0","1","2","3"))   
            col2.text("")
            col2.text("")
...
def developers_content():
    ...
    st.subheader("Ensembled Algorithm (RandomForest + SVM + KNN)")
    st.code('''
    ...
    clf21 = RandomForestClassifier(min_samples_split=20,max_features="log2",bootstrap=False,class_weight="balanced")
    clf22 = KNeighborsClassifier(n_neighbors=best_k)
    clf23 = SVC(kernel='rbf',probability=True)

    eclf2 = VotingClassifier(estimators=[('rf', clf21), ('knn', clf22), ('svc', clf23)],voting='hard')

    params = {'svc__C': [1.0, 100.0], 'rf__n_estimators': [20, 100],'rf__max_features':["log2"]}

    grid2 = GridSearchCV(estimator=eclf2, param_grid=params, cv=50)
...
col2.write("Using Ensembled Algorithm (RandomForest + SVM + KNN )")
st.title("Heart Disease Prediction ")
st.write("Using Ensembled Algorithm (RandomForest + SVM + KNN )")
st.markdown("***")
